...
export const PredictionPanel: React.FC<PredictionPanelProps> = ({ currentScore }) => {
  ...
  const predictions = useMemo(() => {
    return MAJORS_2025.map(major => {
      ...
      return { ...major, status, statusColor, diff };
    }).filter(m => 
  ...
  return (
    <div className="glass-card" style={{ marginTop: '2rem', gridColumn: '1 / -1' }}>
      ...
      <p className="helper-text" style={{ marginBottom: '1.5rem', fontSize: '0.95rem' }}>
        ...
        <br/><em style={{color: '#dc2626'}}>Lưu ý: Kết quả phân tích dưới đây chỉ mang tính chất tham khảo.</em>
      </p>
      ...
      <div style={{ maxHeight: '500px', overflowY: 'auto', borderRadius: '0.5rem', border: '1px solid var(--border)' }}>
